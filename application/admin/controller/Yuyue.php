...
class Yuyue extends Error{
    ...
    function lists()
    {
        ...
        if($username)
        {
            $map[] = ['username|mobile','like','%'.$username.'%'];
        }
        $limit = input('limit/d',1);
        $page = input('page/d',1);
        $data['data'] = db('yuyue')
                    ->alias('i')
                    ->join('huodong a','a.id=i.huodong_id')
                    ->field('i.id,a.activity_name,i.username,i.mobile,i.shoot_person,i.yuyue_time,i.status,i.backup')
                    ->where($map)
                    ->page($page)
                    ->limit($limit)
                    ->select();
        $data['count'] = db('yuyue')->where($map)->count();
        $data['code'] = 0;
        return $data;
    }
    function add(){
